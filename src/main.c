...
static int cmd_ip(int argc, char** argv, void(*callback)(char* result, int exit_status)) {
	...
	if(!strcmp("add", argv[1])) {
		uint32_t addr = str_to_addr(argv[2]);

		if(!is_uint8(argv[3]))
			return -3;

		uint8_t ni_num = parse_uint8(argv[3]);
...
static int cmd_sa(int argc, char** argv, void(*callback)(char* result, int exit_status)) {
	for(int i = 1; i < argc; i++) {
		if(!strcmp(argv[i], "add")) {
			...
			SA* sa = sa_alloc(NetworkInterface* ni, src_ip, dst_ip, protocol, spi, extensions, crypto_algorithm, auth_algorithm, crypto_key, auth_key);
			if(sa == NULL) {
			...
			SA* sa = sad_sa_get(spi, dst_ip, protocol);
			if(sa == NULL)
				return -1;
...
static int cmd_content(int argc, char** argv, void(*callback)(char* result, int exit_status)) {
	for(int i = 1; i < argc; i++) {
		if(!strcmp(argv[i], "add")) {
			...
			Content* cont = content_alloc(protocol, mode, t_src_ip, t_dst_ip, crypto_algorithm, auth_algorithm);
			if(cont == NULL) {
